...
async function registerRoutes(app) {
    ...
    const { initStorage } = await Promise.resolve().then(() => __importStar(require("./storage")));
    ...
    const contactVerifications = new Map();
    const bookingVerifications = new Map();
    ...
    app.put("/api/admin/bookings/:bookingId/status", async (req, res) => {
        try {
            ...
            const validatedData = shared_schema_1.updateBookingStatusSchema.parse({ bookingId, status });
        ...
        <head>
          <style>
            ...
            .status-badge { display: inline-block; padding: 10px 20px; background: ${validatedData.status === "confirmed"
                ? "#28a745"
                : validatedData.status === "rejected"
                    ? "#dc3545"
                    : validatedData.status === "completed"
                        ? "#20c997"
                        : "#ffc107"}; color: white; border-radius: 5px; font-weight: 600; margin: 20px 0; }
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>${statusMessages[validatedData.status]}</p>
    ...
    app.post("/api/bookings", async (req, res) => {
        try {
            ...
            const validatedData = shared_schema_1.insertBookingSchema.parse(req.body);
    ...
    app.post('/api/contact/verify', async (req, res) => {
        try {
            const validated = shared_schema_1.contactVerifySchema.parse(req.body);
            // rate-limit by email
            const now = Date.now();
            ...
            if (err instanceof zod_1.z.ZodError) {
                return res.status(400).json({ success: false, error: 'Invalid contact data', details: err.issues });
            }
    ...
    app.post('/api/contact/confirm', async (req, res) => {
        try {
            const validated = shared_schema_1.contactConfirmSchema.parse(req.body);
    ...
    app.post('/api/bookings/verify', async (req, res) => {
        ...
        try {
            const validated = shared_schema_1.bookingVerifySchema.parse(req.body);
    ...
    app.post('/api/bookings/confirm', async (req, res) => {
        try {
            ...
            const validated = shared_schema_1.bookingConfirmSchema.parse(req.body);
            const key = validated.email.toLowerCase();
            ...
            if (err instanceof zod_1.z.ZodError) {
                console.error('booking/confirm zod error:', err.issues);
                return res.status(400).json({ success: false, error: 'Invalid booking data', details: err.issues });
            }
    ...
    app.get('/api/debug/storage', (_req, res) => {
        try {
            ...
            const mongooseState = (mongoose_1.default && mongoose_1.default.connection && typeof mongoose_1.default.connection.readyState === 'number') ? mongoose_1.default.connection.readyState : 'unknown';
